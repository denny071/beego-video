...
func RedisGetUserInfo(uid int) (UserInfo, error) {
	...
	// 判断redis是否存在
	exist, err := redis.Bool(conn.Do("exists", redisKey))
	if exist {
		res, _ := redis.Values(conn.Do("hgetall", redisKey))
		err = redis.ScanStruct(res, &user)
	} else {
		o := orm.NewOrm()
		err := o.Raw("SELECT id,name,add_time,avatar FROM user WHERE id = ? LIMIT", uid).QueryRow(&user)
		if err == nil {
			// 保存redis
